...
export const loginSchema = zod.object({
    email: zod.string({required_error: "L'email est requis"})
        .min(1, "L'email ne peut pas être vide")
        .email("L'email doit être valide"), password: zod.string({required_error: "Le mot de passe est requis"})
        .min(8, "Le mot de passe doit contenir au moins 8 caractères")
        .max(128, "Le mot de passe ne peut pas dépasser 128 caractères")
})
...
export const addCampaignSchema = zod.object({
    ...
    allow_overflow: zod.boolean().optional()
})
    .refine(
        (data) => data.image_urls.length > 0,
        {message: "Au moins une images est requise", path: ["image_urls"]}
    )
    .refine(
        (data) => data.image_urls.length < 5,
        {message: "Limite d'images atteinte", path: ["image_urls"]}
    )